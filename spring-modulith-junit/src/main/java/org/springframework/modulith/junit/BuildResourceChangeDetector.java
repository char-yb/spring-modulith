...
final class BuildResourceChangeDetector {
	...
	private static final Set<String> EXACT_PATHS = Set.of(
			...
			// Build entrypoints
			"pom.xml", "build.gradle", "build.gradle.kts", "settings.gradle", "settings.gradle.kts", "init.gradle",
			"init.gradle.kts", "gradle.properties",

			// Gradle metadata
			"gradle/libs.versions.toml", "gradle/wrapper/gradle-wrapper.properties");

	private static final Set<String> SUFFIXES = Set.of("/pom.xml", "/build.gradle", "/build.gradle.kts",
			"/settings.gradle", "/settings.gradle.kts", "/init.gradle", "/init.gradle.kts", "/gradle.properties");
	...
	static boolean isBuildResource(String path) {
		...
		return EXACT_PATHS.contains(normalizedPath)
				...
				|| isGradleDirectoryScript(normalizedPath)
				|| DIRECTORY_PREFIXES.stream().anyMatch(it -> normalizedPath.equals(it) || normalizedPath.startsWith(it + "/"));
	}

	private static boolean isGradleDirectoryScript(String path) {
		return path.startsWith("gradle/") && (path.endsWith(".gradle") || path.endsWith(".gradle.kts"));
	}
}
